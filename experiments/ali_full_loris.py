...
from ali.streams import create_loris_data_streams
...
def create_model_brick():
    ...
    ali.initialize()
    raw_marginals, = next(
        create_loris_data_streams(500, 500)[0].get_epoch_iterator())
...
def create_main_loop(save_path):
    ...
    streams = create_loris_data_streams(BATCH_SIZE, MONITORING_BATCH_SIZE)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train ALI on LORIS")
    parser.add_argument("--save-path", type=str, default='ali_loris_full.tar',
